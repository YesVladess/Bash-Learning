...
# Пример 4-1. Присваивание значений переменным и подстановка значений переменных

# Присваивание значений переменным и подстановка значений переменных
a=375
hello=$a
#-------------------------------------------------------------------------
...
echo $hello
echo ${hello} # Идентично предыдущей строке.
echo "$hello"
echo "${hello}"
echo
hello="A B  C   D"
...
echo
echo '$hello'  # $hello
# Внутри одинарных кавычек не производится подстановка значений переменных,
#+ т.е. "$" интерпретируется как простой символ.
...
#+ что сброс переменной, хотя конечный результат -- тот же (см. ниже).
# --------------------------------------------------------------
#  Допускается присваивание нескольких переменных в одной строке,
...
uninitialized_variable=   #  Объявление неинициализированной переменной
